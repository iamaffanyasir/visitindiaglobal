...
const FeaturedDestinations = () => {
  const [destinations, setDestinations] = useState(featuredDestinationsData);
  const [selectedDestination, setSelectedDestination] = useState(null);
  const [modalOpen, setModalOpen] = useState(false);

  useEffect(() => {
    // Only call API if you have backend setup
    // For now, use static data
    setDestinations(featuredDestinationsData);
  }, []);

  const handleScroll = (direction) => {
    const container = document.querySelector('.fd-carousel');
    const scrollAmount = direction === 'left' ? -1100 : 1100; // Adjusted for 4 tiles
  ...
  return (
    <div className="fd-container">
      <div className="fd-header">
        <h4 className="fd-subtitle">FEATURED PLACES</h4>
        <h2 className="fd-title">Popular Destinations</h2>
      </div>
